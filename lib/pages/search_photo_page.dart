...
class SearchPhoto extends StatefulWidget {
  static const String id = "search_photo";
  User? user;
  String? search;
  int? index;
  SearchPhoto({Key? key,this.user,this.search,this.index}) : super(key: key);
...
class _SearchPhotoState extends State<SearchPhoto> {
  ...
  void _userList() async {
    ...
    await Network.GET(Network.API_LIST_SEARCH, Network.paramsSearch(widget.index!, widget.search.toString())).then((response) {
      Log.d(response!);
  ...
  void _showResponse(String response) {
    List<User> list = Network.parseSearchParse(response);
    _list.clear();
  ...
  void _fetchPosts() async {
    int pageNumber = (_list.length ~/ postsLength + 1)+widget.index!;
    String? response =
    await Network.GET(Network.API_LIST_SEARCH, Network.paramsSearch(pageNumber,widget.search.toString()));
                        ...
                        leading: ClipRRect(
                          ...
                          child: CachedNetworkImage(
                            ...
                            placeholder: (context, url) => AspectRatio(
                              aspectRatio: widget.user!.width! / widget.user!.height!,
                              child: ClipRRect(
          ...
          GestureDetector(
            ...
            onTap: ()async{
              await Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context){
                return SearchPhoto(user: _list[index],search: widget.search.toString(),index: index,);
              }));
            },
            child: ClipRRect(
              borderRadius: BorderRadius.circular(15),
